...
void Control::askAboutDocument() {
    ...
    // Lazy‑construct a chat controller bound to the main window. The concrete
    // view (ChatWebViewGtk) is owned by MainWindow; here we only
    // trigger the "ask" flow. For now we simply reuse the document‑ask dialog
    // to capture a question and then delegate to ChatController.
    GtkWindow* parent = GTK_WINDOW(win->getWindow());

    GtkWidget* dialog =
            gtk_dialog_new_with_buttons(_("Ask about document"), parent, GTK_DIALOG_MODAL, _("_Cancel"),
                                        GTK_RESPONSE_CANCEL, _("_Ask"), GTK_RESPONSE_ACCEPT, nullptr);
    ...
    if (auto* controller = win->getChatController()) {
        controller->sendUserMessage(question);
    }
}

void Control::ensureLLMModel(std::function<void(bool, const std::string&)> callback) {
    // Legacy API retained for compatibility; the new chat stack manages models
    // internally. For now, signal "not available" to any old callers.
    if (callback) {
        callback(false, "ensureLLMModel is deprecated; use the new chat architecture.");
    }
}
